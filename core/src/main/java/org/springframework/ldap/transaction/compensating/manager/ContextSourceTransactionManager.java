 ...
 * <em>compensating</em> transactions instead. Should a transaction need to be rolled
 ...
 * <p>
 * <b>NOTE:</b> The transactions provided by this TransactionManager are all <i>client
 * side</i> and are by no means 'real' transactions, in the sense that we know them in the
 * ordinary database world, e.g.:
 * <ul>
 * <li>Should the transaction failure be caused by a network failure, there is no way
 * whatsoever that this TransactionManager can restore the database state. In this case,
 * all possibilities for rollback will be utterly lost.</li>
 * <li>Transaction isolation is not provided, i.e. entries participating in a transaction
 ...
 * undetermined, and may in the worst case result in total failure.</li>
 * </ul>
 * <p>
